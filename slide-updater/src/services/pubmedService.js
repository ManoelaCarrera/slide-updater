...
export async function searchPubMed(keywords, year) {
    try {
        // Check cache first
        ...
        const response = await withRetry(() => fetch(url.toString()), { maxRetries: 3, initialDelayMs: 100, maxDelayMs: 5000 });
        if (!response.ok)
 ...
 */
export async function searchMultipleSources(keywords, year) {
    try {
        const results = [];
