...
export class DesignAnalyzer {
    constructor() {
         ...
         */
        Object.defineProperty(this, "designSystemColors", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: [
    ...
    analyze(content, title) {
        ...
        const overallScore = this.calculateDesignScore(wordCount, hasStructure, hasVisuals, suggestions.length);
        return {
            suggestions,
            summary: {
                wordCount,
     ...
     */
    detectVisualPlaceholders(content) {
     ...
     */
    createTextHeavySuggestion(wordCount) {
        return {
            id: generateId(),
            type: 'text-heavy',
            message: `Este slide contém ${wordCount} palavras, excedendo o limite recomendado de 150 palavras.`,
            recommendation: 'Divida o conteúdo em múltiplos slides ou crie estrutura visual (bullets, diagramas). Mantenha máximo 150 palavras por slide para legibilidade.',
            severity: 'high',
            actionable: true,
     ...
     */
    createMissingHierarchySuggestion(wordCount) {
        return {
            id: generateId(),
            type: 'missing-hierarchy',
            message: `Conteúdo sem estrutura hierárquica clara (${wordCount} palavras em bloco contínuo).`,
            recommendation: 'Use bullets (•), números, parágrafos curtos ou seções com cabeçalhos. Hierarquia visual melhora retenção e legibilidade. Estruture em 3-5 pontos principais.',
            severity: 'high',
            actionable: true,
     ...
     */
    createMissingVisualSuggestion() {
        return {
            id: generateId(),
            type: 'missing-visual',
            message: 'Slide possui apenas conteúdo textual, sem elementos visuais.',
            recommendation: 'Adicione um elemento visual: gráfico, diagrama, foto, ícone ou infográfico. Referências visuais aumentam compreensão e retenção em até 65%.',
            severity: 'medium',
            actionable: true,
     ...
     */
    createSuggestedLayoutSuggestion(content) {
        let layout = 'balanced';
     ...
     */
    calculateDesignScore(wordCount, hasStructure, hasVisuals, suggestionCount) {
        let score = 100;
 ...
 */
export function analyzeDesign(content, title) {
