...
export function DesignSuggestions({ slide, project }: DesignSuggestionsProps) {
  ...
  return (
    <div className="p-4 space-y-4">
      {suggestions.map(suggestion => (
        <Card key={suggestion.id} className={getSeverityColor(suggestion.severity)}>
          <CardBody>
            <div className="mb-2">
            ...
            <div className="flex gap-2 mt-3">
              ...
              <Button
                size="sm"
                variant="ghost"
                onClick={() => removeDesignSuggestion(project.id, slide.id, suggestion.id)}
              >
                ✕ Ignorar
              </Button>
            </div>
      ...
      {slide.designNotes.appliedChanges.length > 0 && (
        <div className="mt-6 pt-4 border-t border-neutral-200">
          <h3 className="font-semibold text-neutral-900 mb-2">Mudanças Aplicadas</h3>
          <ul className="space-y-1">
            {slide.designNotes.appliedChanges.map((change, i) => (
