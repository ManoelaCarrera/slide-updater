...
export function Navbar() {
    ...
    return (<nav className="bg-white border-b border-neutral-200 shadow-sm">
      <div className="max-w-full px-4 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <Button variant="secondary" size="sm" onClick={undo} disabled={!canUndo()} title="Desfazer (Ctrl+Z)" aria-label="Desfazer última ação">
            ↶
          </Button>
          <Button variant="secondary" size="sm" onClick={redo} disabled={!canRedo()} title="Refazer (Ctrl+Y)" aria-label="Refazer ação desfeita">
            ↷
          </Button>
