...
export function SlideEditor() {
    ...
    if (!project || !slide) {
        return (<div className="flex-1 flex items-center justify-center text-neutral-600">
    ...
    return (<div className="flex-1 flex flex-col overflow-hidden">
      ...
      <div className="border-b border-neutral-200 bg-white px-4 py-2 flex gap-2" role="tablist">
        {['edit', 'literature', 'design', 'preview'].map(tab => (<button key={tab} onClick={() => setActiveTab(tab)} role="tab" aria-selected={activeTab === tab} aria-label={`Aba ${tab === 'edit' ? 'Editar' : tab === 'literature' ? 'Literatura' : tab === 'design' ? 'Design' : 'Preview'}`} className={`px-3 py-2 rounded-md text-sm font-medium transition-colors ${activeTab === tab
      ...
      <div className="flex-1 overflow-auto">
        {activeTab === 'edit' && (<div className="p-4 space-y-4">
            <div>
              <label htmlFor="slide-title" className="block text-sm font-medium text-neutral-900 mb-2">
                Título do Slide
              </label>
              <input id="slide-title" type="text" value={slide.title} onChange={(e) => updateSlide(project.id, slide.id, { title: e.target.value })} className="w-full px-3 py-2 border border-neutral-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-600" aria-label="Título do slide"/>
            </div>

            <div>
              <label className="block text-sm font-medium text-neutral-900 mb-2">Conteúdo</label>
              <RichTextEditor value={pendingContent || slide.currentContent} onChange={handleContentChange} placeholder="Cole ou digite o conteúdo do slide..."/>
            </div>

            <div className="flex gap-2">
              <Button variant="primary" onClick={handleSearchLiterature} isLoading={isSearching} aria-label="Buscar literatura relacionada ao conteúdo do slide">
                🔍 Buscar Literatura
              </Button>
              <Button variant="secondary" onClick={handleGenerateDesignSuggestions} aria-label="Gerar sugestões de design para o slide">
                ✨ Sugestões de Design
              </Button>
            </div>
          </div>)}

        {activeTab === 'literature' && (<LiteraturePanel slide={slide} project={project}/>)}

        {activeTab === 'design' && (<DesignSuggestions slide={slide} project={project}/>)}

        {activeTab === 'preview' && (<div className="p-4">
            <Card>
              <CardHeader>
              ...
              <CardBody>
                <div className="prose max-w-none">
                  {slide.currentContent.split('\n').map((para, i) => (<p key={i} className="mb-3 text-neutral-900">
                      {para}
                    </p>))}
                </div>
              </CardBody>
