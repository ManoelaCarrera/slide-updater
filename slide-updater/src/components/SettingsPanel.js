        ...
        <Card>
          ...
          <CardBody className="space-y-4">
            <div>
              ...
              {!showApiKeyInput ? (<div className="flex items-center gap-2">
                  {isSaved ? (<>
                      <div className="flex-1 px-3 py-2 bg-green-50 border border-green-200 rounded text-sm text-green-700" role="status" aria-live="polite">
                        ✓ Scopus habilitado ({scopusApiKey})
                      </div>
                      <Button size="sm" variant="secondary" onClick={() => setShowApiKeyInput(true)} aria-label="Editar chave de API Scopus">
                        Editar
                      </Button>
                      <Button size="sm" variant="ghost" onClick={handleRemoveApiKey} aria-label="Remover chave de API Scopus">
                        Remover
                      </Button>
                    </>) : (<Button size="sm" variant="primary" onClick={() => setShowApiKeyInput(true)} aria-label="Adicionar chave de API Scopus">
                      Adicionar Chave
                    </Button>)}
                </div>) : (<div className="space-y-2">
                  <input type="password" placeholder="Cole sua chave de API Scopus aqui..." value={scopusApiKey} onChange={(e) => setScopusApiKeyLocal(e.target.value)} className="w-full px-3 py-2 border border-neutral-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-600 text-sm" aria-label="Entrada de chave de API Scopus"/>
                  <p className="text-xs text-neutral-500">
                    Sua chave não será compartilhada e fica armazenada localmente no navegador.
                  </p>
                  <div className="flex gap-2">
                    <Button size="sm" variant="primary" onClick={handleSaveApiKey} aria-label="Salvar chave de API">
                      Salvar
                    </Button>
