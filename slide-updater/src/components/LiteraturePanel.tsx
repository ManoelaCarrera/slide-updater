...
export function LiteraturePanel({ slide, project }: LiteraturePanelProps) {
  ...
  return (
    <div className="p-4 space-y-3" role="region" aria-label="Lista de referências literárias">
      {slide.literatureUpdates.map(lit => (
        <Card key={lit.id} className={lit.approved ? 'border-green-200 bg-green-50' : ''}>
          <CardBody>
            <div className="mb-2">
            ...
            <div className="flex gap-2 mt-3">
              {!lit.approved ? (
                <>
                  <Button
                    size="sm"
                    variant="primary"
                    onClick={() => handleApprove(lit.id)}
                    aria-label={`Aprovar referência: ${lit.title}`}
                  >
                    ✓ Aprovar
                  </Button>
                  <Button
                    size="sm"
                    variant="secondary"
                    onClick={() => handleInsertIntoSlide(lit)}
                    aria-label={`Inserir referência no slide: ${lit.title}`}
                  >
                    + Inserir
                  </Button>
                </>
              ...
              <Button
                ...
                aria-label={`Remover referência: ${lit.title}`}
              >
                🗑️ Remover
              </Button>
            </div>
