    ...
    return (<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <Card className="w-96">
        <CardBody>
          ...
          <div onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop} className={`border-2 border-dashed rounded-lg p-8 text-center transition-colors ${isProcessing
            ...
            <input ref={fileInputRef} type="file" multiple accept=".txt,.pdf,.pptx,.ppt" disabled={isProcessing} onChange={(e) => {
            Array.from(e.target.files || []).forEach(file => handleFileUpload(file));
        }} className="hidden"/>
            <Button variant="primary" size="sm" onClick={() => fileInputRef.current?.click()} disabled={isProcessing} isLoading={isProcessing} className="mt-4">
